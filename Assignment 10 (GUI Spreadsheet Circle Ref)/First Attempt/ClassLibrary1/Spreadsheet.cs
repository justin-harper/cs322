...
namespace Spreadsheet
{
    public class Spreadsheet
    {
        ...
        *************************************************************************************/
        ...
        internal List<string> visitedCells = new List<string>();
        ...
        private void evaluateExpression(CellChild cell)
        {
            ...
            for (int j = 0; j < expression.Length; j++)
            {
                if (expression.ToUpper()[j] >= 'A' && expression.ToUpper()[j] <= 'Z')
                {
                    ...
                    {
                        cell.setText("!SELFREF");
                    }
                    
                    //this cell depends on variable cell
                    cell.addIDependOn(variable.Name);

                    //if variable cell depends on me we have a circular ref
      ...
      /******/                 cell.setText(variable.Text);
            ...
            try
            {
                cell.setText(expTree.Evaluate().ToString());
            }
            catch (DivideByZeroException e)
            {
                cell.setText(e.Message);
            }          
            return;
        }
        ...
        *************************************************************************************************/
        ...
        private void updateCellsThatDependOnMe(CellChild c)
        {
            ...
            if(visitedCells.Contains(c.Name) == false)
            {
                visitedCells.Add(c.Name);
            }
            else
            {
            ...
            for (int i = 0; i < c._DependOnMe.Count; i++)                
            {
                ...
                if (visitedCells.Contains(x.Name))
                {
                    x.setText("!CREF");
                    throw new Exception("!CREF");
                }
                else
                {
                    x.setText(x.Value);
        ...
        *************************************************************************************/
        ...
        public bool CircleRef(CellChild c1, CellChild c2)
        {
            //if c1 depends on anything c2 depends on
            //we have a circular ref
            foreach(var d in c1._IdependOn)
            {
                if (c2._IdependOn.Contains(d))
                {
                    return true;
                }
            }
